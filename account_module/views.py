    ...
    def post(self, request):
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                ...
                login_form.add_error('email', 'ایمیل وارد شده صحیح نمیباشد')
        context = {
            'login_form': login_form
        }
        return render(request, 'account_module/login.html', context)
...
class ForgetPasswordView(View):
    def get(self, request: HttpRequest):
        ...
        context = {
            'forgot_pass_form': forget_pass_form
        }
...
class ResetPasswordView(View):
    ...
    def post(self, request: HttpRequest, active_code):
        ...
        if reset_password.is_valid():
            if user is None:
                return redirect(reverse('login_page'))
            user_new_pass = reset_password.cleaned_data.get('password')
            user.set_password(user_new_pass)
            user.email_active_code = get_random_string(72)
            user.is_active = True
            user.save()
            return redirect(reverse('login_page'))
        context = {'reset_pass_form': reset_password,
                   'user': user}
        return render(request, 'account_module/reset_password.html', context)
